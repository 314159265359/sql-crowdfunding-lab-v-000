...
"SELECT projects.title, sum(pledges.amount) FROM projects JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING sum(pledges.amount) > projects.fundong_goal"
end
